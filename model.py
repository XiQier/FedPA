...
class MLP(torch.nn.Module):
    def __init__(self, config, user_features, video_features, selected_user_features, selected_video_features):
        ...
        #输入层维度 输入层维度 = 用户特征数*嵌入维度 + 视频特征数*嵌入维度
        ...
        for idx, (in_size, out_size) in enumerate(zip(self.layers[:-1], self.layers[1:])):
            ...
            """group lora"""
            for lora_user_feature in config['lora_user_features']:# 关注数组 活跃程度组
                ...
                for lora_user_feature_value in lora_user_feature_values:#每个用户的关注数
    ...
    def forward(self, user_indices, item_indices, lora_user_feature_values):
        ...
        for idx, _ in enumerate(range(len(self.fc_layers))):
            ...
            gate_vector = torch.nn.Softmax(dim=1)(gate_vector)
